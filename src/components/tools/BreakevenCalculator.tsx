    ...
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="space-y-2">
          <Label>{t("internals.breakevenCalculator.labels.entryPriceDollar")}</Label>
          <Input type="number" placeholder="50000" value={entryPrice} onChange={(e) => setEntryPrice(e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t("internals.breakevenCalculator.labels.tradingFeePercent")}</Label>
          <Input type="number" placeholder="0.1" step="0.01" value={feePercent} onChange={(e) => setFeePercent(e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t("internals.breakevenCalculator.labels.leverage")}</Label>
          <Input type="number" placeholder="1" value={leverage} onChange={(e) => setLeverage(e.target.value)} />
        </div>
      </div>
      <Button onClick={calculate} disabled={!entryPrice} className="bg-accent text-accent-foreground hover:bg-gold-dark">{t("internals.breakevenCalculator.buttons.calculateBreakeven")}</Button>
      {result && (
