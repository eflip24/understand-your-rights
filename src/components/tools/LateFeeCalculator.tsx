    ...
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label>{t("internals.lateFeeCalculator.labels.principalAmountDollar")}</Label>
          <Input type="number" placeholder="1000" value={amount} onChange={(e) => setAmount(e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t("internals.lateFeeCalculator.labels.feeRatePercent")}</Label>
          <Input type="number" placeholder="1.5" step="0.1" value={rate} onChange={(e) => setRate(e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t("internals.lateFeeCalculator.labels.daysLate")}</Label>
          <Input type="number" placeholder="30" value={days} onChange={(e) => setDays(e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t("internals.lateFeeCalculator.labels.rateType")}</Label>
      ...
      <Button onClick={calculate} disabled={!amount || !rate || !days} className="bg-accent text-accent-foreground hover:bg-gold-dark">{t("internals.lateFeeCalculator.buttons.calculateLateFee")}</Button>
      {result && (
