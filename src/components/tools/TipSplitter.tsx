  ...
  return (
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-3">
        <div><Label>Subtotal ($)</Label><Input type="number" value={bill} onChange={e => setBill(e.target.value)} /></div>
        <div><Label>Tax ($)</Label><Input type="number" value={tax} onChange={e => setTax(e.target.value)} /></div>
        <div><Label>Tip %</Label><Input type="number" value={tip} onChange={e => setTip(e.target.value)} disabled={serviceCharge} /></div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Label htmlFor="sc" className="text-sm">Mandatory service charge already included (skip tip)</Label>
      </div>
      <Button onClick={calc} disabled={!bill}>Calculate Split</Button>
      {result && (
