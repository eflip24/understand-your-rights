// Slug ↔ state-name registry for /alimony-calculator/{state} landing pages.
// State names must match keys in ALIMONY_STATE_FORMULAS (AlimonyCalculator.tsx).

export interface AlimonyStateEntry {
  slug: string;
  name: string;
  abbr: string;
}

export const ALIMONY_STATES: AlimonyStateEntry[] = [
...
export const getAlimonyStateBySlug = (slug: string) =>
  ALIMONY_STATES.find((s) => s.slug === slug.toLowerCase()) ?? null;
