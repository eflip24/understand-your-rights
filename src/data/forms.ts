...
export const legalForms: LegalFormDef[] = [
  ...
  {
    slug: "small-claims-demand-packet",
    title: "Small Claims Demand Letter & Filing Prep Packet",
    shortDescription:
      "Send the pre-suit demand letter most courts expect, then walk into the clerk's office with your claim amount, court, filing fee and evidence list ready.",
    category: "personal",
    price: 7.99,
    ...
    steps: [
      {
        ...
        description: "Name the defendant exactly as they do business — a wrong legal name is the most common reason a small claims judgment can't be collected.",
        fields: [
          { id: "claimantName", label: "Your full legal name", type: "text", required: true },
          { id: "claimantAddress", label: "Your mailing address", type: "textarea", required: true },
          { id: "claimantPhone", label: "Your phone", type: "text" },
          { id: "claimantEmail", label: "Your email", type: "email" },
          { id: "defendantName", label: "Defendant's full legal name or registered business name", type: "text", required: true,
            help: "For a business, use the name on the Secretary of State registration, plus 'd/b/a' and the trading name." },
          { id: "defendantAddress", label: "Defendant's address for service", type: "textarea", required: true },
          { id: "defendantType", label: "Defendant is a", type: "select", required: true,
            options: [
              { value: "individual", label: "Individual" },
              { value: "soleProprietor", label: "Sole proprietor" },
              { value: "llcCorp", label: "LLC or corporation" },
              { value: "landlord", label: "Landlord / property manager" },
              { value: "contractor", label: "Contractor / tradesperson" },
            ] },
          { id: "registeredAgent", label: "Registered agent for service (LLC / corporation)", type: "text",
            showWhen: (d) => d.defendantType === "llcCorp",
            help: "Look this up free on your Secretary of State's business search. Serving the agent is what makes the judgment enforceable." },
        ],
      },
      {
        id: "claim",
        title: "What happened and what you are owed",
        fields: [
          { id: "filingState", label: "State where you will file", type: "usState", required: true },
          { id: "county", label: "County / court district", type: "text", required: true },
          { id: "claimType", label: "Type of claim", type: "select", required: true,
            options: [
              { value: "unpaidDebt", label: "Unpaid invoice or loan" },
              { value: "securityDeposit", label: "Withheld security deposit" },
              { value: "propertyDamage", label: "Property damage" },
              { value: "badWork", label: "Defective or unfinished work" },
              { value: "goodsNotDelivered", label: "Goods paid for but not delivered" },
              { value: "breachOfContract", label: "Other breach of contract" },
            ] },
          { id: "incidentDate", label: "Date the dispute arose", type: "date", required: true,
            help: "The statute of limitations usually runs from this date — written contracts commonly get 4–6 years, oral contracts 2–4." },
          { id: "principalAmount", label: "Principal amount claimed ($)", type: "number", required: true },
          { id: "interestClaimed", label: "Interest or late fees claimed ($)", type: "number" },
          { id: "costsClaimed", label: "Filing and service costs to be added ($)", type: "number" },
          { id: "depositMultiplier", label: "Claiming statutory damages for a wrongfully withheld deposit", type: "checkbox",
            showWhen: (d) => d.claimType === "securityDeposit",
            help: "Many states allow 2× or 3× the deposit plus fees when a landlord misses the statutory return deadline." },
          { id: "facts", label: "What happened, in plain order of events", type: "textarea", required: true,
            placeholder: "On 12 March 2026 I paid $2,400 by bank transfer for a bathroom refit. Work stopped on 2 April with the tiling unfinished. I asked for completion or a refund by text on 9, 14 and 21 April…" },
          { id: "priorRequests", label: "Attempts you already made to resolve it", type: "textarea",
            placeholder: "Three text messages, one phone call on 21 April, and an email on 2 May — all unanswered." },
        ],
      },
      {
        id: "demand",
        title: "Your demand",
        description: "Most small claims courts either require or strongly favour a written demand before filing. This is the letter you send first.",
        fields: [
          { id: "demandAmount", label: "Total amount demanded ($)", type: "number", required: true },
          { id: "responseDays", label: "Days you give them to pay", type: "select", required: true,
            options: [
              { value: "7", label: "7 days" },
              { value: "10", label: "10 days (most common)" },
              { value: "14", label: "14 days" },
              { value: "30", label: "30 days" },
            ] },
          { id: "paymentMethod", label: "How they should pay", type: "textarea",
            placeholder: "Certified cheque to the address above, or bank transfer to the details I will provide on request." },
          { id: "settlementOpen", label: "I am willing to accept a payment plan or reduced lump sum", type: "checkbox" },
          { id: "deliveryMethod", label: "How you will send the demand", type: "select",
            options: [
              { value: "certified", label: "Certified mail, return receipt requested" },
              { value: "certifiedAndEmail", label: "Certified mail plus email" },
              { value: "hand", label: "Hand delivery with a witness" },
            ],
            help: "Certified mail with the green card is the proof judges expect. Keep the receipt — it becomes an exhibit." },
        ],
      },
      {
        id: "evidence",
        title: "Filing prep and evidence",
        description: "This section is printed as your court-day checklist, not as part of the letter sent to the defendant.",
        fields: [
          { id: "evidenceList", label: "Evidence you can produce", type: "textarea",
            placeholder: "Signed estimate, bank transfer confirmation, 14 text messages, 6 photographs of the unfinished tiling, two written quotes to complete the work." },
          { id: "witnessNames", label: "Witnesses (name and what they saw)", type: "textarea" },
          { id: "hasWrittenContract", label: "There is a written contract or invoice", type: "checkbox" },
          { id: "serviceMethodPlanned", label: "Planned method of service", type: "select",
            options: [
              { value: "sheriff", label: "Sheriff / constable" },
              { value: "processServer", label: "Private process server" },
              { value: "certifiedMail", label: "Certified mail by the clerk" },
            ] },
          { id: "feeWaiver", label: "I intend to apply for a filing fee waiver (in forma pauperis)", type: "checkbox" },
          { id: "signatureDate", label: "Date of the demand letter", type: "date", required: true },
        ],
      },
    ],
    relatedForms: ["demand-letter", "promissory-note", "security-deposit-receipt"],
  },
  {
    slug: "debt-validation-letter",
    title: "Debt Validation Letter (FDCPA)",
    shortDescription:
      "Force a collection agency to prove the debt is yours before they can keep collecting or report it — your 30-day right under the Fair Debt Collection Practices Act.",
    category: "personal",
    price: 4.99,
    lastUpdated: "2026-08-16",
    isFeatured: true,
    ...
    steps: [
      {
        ...
        fields: [
          ...
          { id: "consumerAddress", label: "Your mailing address", type: "textarea", required: true },
          { id: "lastFourSsn", label: "Last 4 digits of your SSN (optional)", type: "text",
            help: "Only include this if the collector already has your file and you need them to match it. Never send the full number." },
        ],
      },
      {
        id: "collector",
        title: "The collection agency",
        fields: [
          ...
          { id: "accountReference", label: "Account or reference number on their letter", type: "text" },
          { id: "originalCreditor", label: "Original creditor named (if any)", type: "text" },
          { id: "amountClaimed", label: "Amount they claim you owe ($)", type: "number" },
          { id: "firstContactDate", label: "Date of their first written contact", type: "date", required: true,
            help: "Your validation request is strongest when it is postmarked within 30 days of this date — collection must then pause until they validate." },
        ],
      },
      {
        id: "requests",
        title: "What you are demanding they produce",
        fields: [
          { id: "requestAgreement", label: "A copy of the original signed agreement", type: "checkbox" },
          { id: "requestStatement", label: "A full account statement showing how the balance was calculated", type: "checkbox" },
          { id: "requestChainOfTitle", label: "Proof of the chain of assignment from the original creditor", type: "checkbox" },
          { id: "requestLicence", label: "Proof the agency is licensed to collect in my state", type: "checkbox" },
          { id: "disputeAmount", label: "I dispute the amount claimed", type: "checkbox" },
          { id: "disputeIdentity", label: "I dispute that this debt is mine", type: "checkbox" },
          { id: "ceaseCalls", label: "Demand that all contact be in writing only", type: "checkbox",
            help: "15 U.S.C. § 1692c(c) lets you require written-only contact, or stop contact entirely." },
          { id: "timeBarredConcern", label: "I believe this debt may be past the statute of limitations", type: "checkbox",
            help: "Never make a payment or acknowledge a time-barred debt in writing — in many states that restarts the clock." },
          { id: "additionalNotes", label: "Anything else to state", type: "textarea" },
          { id: "letterDate", label: "Date of this letter", type: "date", required: true },
        ],
      },
    ],
    relatedForms: ["demand-letter", "small-claims-demand-packet"],
  },
];
